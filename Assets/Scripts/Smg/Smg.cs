...
public class Smg : MonoBehaviour
{
    ...
    void Update()
    {
        if (scriptHand.currentAttachedObject != null)
        {
            if (scriptHand.currentAttachedObject.GetComponent<Smg>())
            {
                ...
                if (Input.GetKey("space")) /*(buttonGrabPinch.GetStateDown(Pos.inputSource) && OnPress == false)*/ //изменить кнопку на кнопку на контроллере (включить)
                {
                    if(Time.time > nextShoot)
                    {
                        if (magazine.GetComponent<SmgMagazine>().ammo > 0 && SmgParams.isEmptyMagazine == false && hasSlide)
                        {
        ...
        //if (buttonGrabGrip.GetStateUp(Pos.inputSource)) { OnPress = false; } //Включить
        if (Input.GetKeyUp("space")) { OnPress = false; }
    }
    ...
    void Reload()
    {
        if (/*buttonGrabGrip.GetStateDown(Pos.inputSource)*/ Input.GetKeyDown(KeyCode.R) && OnPress == false)
        {
